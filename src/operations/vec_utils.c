...
void get_color_helper(t_lihgt_color *light_col, t_material *material, t_compose *comp, t_light *light)
{
    light_col->diffuse = ft_multiply_color_scalar(light_col->eff_color, material->diffuse * light_col->light_dot_normal);
    light_col->reflect_dot_camera = vector_dot(reflect_vector(negate_vector(light_col->light_dir_normal),
    ...
    if (light_col->reflect_dot_camera <= EPSILON)
        ...
        light_col->specular = ft_multiply_color_scalar(light->color,
                    material->specular * powf(light_col->reflect_dot_camera, material->shininess));
}
...
t_color get_lighting_color(t_material *material, t_light *light, t_compose *comp, t_color base_color)
{
    ...
    light_col->ambient = clamp_color(ft_multiply_color_scalar(light_col->eff_color, material->ambient));
    res = light_col->ambient;
    if (comp->shadow)
        return (free(light_col), res);
    light_col->light_dir_normal = vector_normilze(vector_sub(light->coordinate, comp->point));
    light_col->light_dot_normal = vector_dot(light_col->light_dir_normal, comp->normv);
